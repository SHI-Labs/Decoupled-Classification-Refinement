...
class DCRAccMetric(mx.metric.EvalMetric):
    def __init__(self, cfg):
        super(DCRAccMetric, self).__init__('DCRAcc')
        self.e2e = cfg.TRAIN.END2END
        self.ohem = cfg.TRAIN.ENABLE_OHEM
        self.pred, self.label = get_rcnn_names(cfg)

    def update(self, labels, preds):
        pred = preds[self.pred.index('dcr_cls_prob')]
        # TODO: potentially will be dcr labels if resample
        if self.ohem or self.e2e:
            label = preds[self.pred.index('dcr_label')]
        else:
            raise ValueError('undefined dcr label')
